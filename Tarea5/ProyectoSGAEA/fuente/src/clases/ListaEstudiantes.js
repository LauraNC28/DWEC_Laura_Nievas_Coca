...
console.log("Estudiantes actuales:", this.#estudiantes.map(e => ({ id: e.id, nombre: e.nombre })));
            const estudiante = this.#estudiantes.find(e => e.id === Number(id));
            if (!estudiante) {
    ...
    eliminarEstudiante(id) {
        try {
            ...
            this.#estudiantes = this.#estudiantes.filter(e => e.id !== Number(id));
    ...
    obtenerPromedioGeneral() {
        try {
            ...
            return Math.round(promedios.reduce((sum, prom) => sum + prom, 0) / promedios.length);
    ...
    añadirCalificacion(id, nombreAsignatura, calificacion) {
        try {
            ...
            const estudiante = this.obtenerEstudiantePorId(Number(id));
            if (!estudiante) throw new Error("Estudiante no encontrado.");
            
            // Buscar la asignatura en la lista de asignaturas
            if (!estudiante.estaMatriculado(nombreAsignatura)) {
                throw new Error("El estudiante no está matriculado en esta asignatura.");
            }
            
            estudiante.agregarCalificacion({ nombre: nombreAsignatura }, calificacion);
